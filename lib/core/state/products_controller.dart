...
class ProductsController extends ChangeNotifier {
  ...
  // vira false assim que o cache é pintado (cache-first): serve pra um sync em
  // segundo plano não disparar um load() concorrente enquanto a rede ainda
  // está voltando.
  ...
  Future<void> load() async {
    // Evita loads sobrepostos (ex.: um sync em segundo plano disparando load()
    ...
    // Cache-first: pinta na hora o que já está salvo localmente (se houver),
    // pra tela não ficar num spinner esperando a API. A resposta da rede
    // abaixo substitui isso assim que chegar.
    ...
    try {
      ...
      // Sem conexão: se já pintamos o cache acima, só marca offline; senão,
      // não há nada pra mostrar e vira tela de erro.
      if (cached != null) {
        _isOffline = true;
      } else {
