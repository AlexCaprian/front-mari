...
class LocalCache {
  ...
  /// Grava a lista só se ainda não houver nada salvo nessa chave. Usado quando
  /// uma tela carrega dados do mês online (Início, Relatório) e queremos
  /// garantir um fallback offline, sem sobrescrever um cache já existente —
  /// que pode ser mais completo (outros meses) ou conter mutações feitas
  /// offline ainda não sincronizadas.
  Future<void> saveListIfAbsent(
    String key,
    List<Map<String, dynamic>> items,
  ) async {
    final existing = await readList(key);
